...
function image_secure_key_copy()
{
    ...
    if vmx_is_enable_boot_flow; then
        if [ "${VMX_TYPE}" == "ultra" ]; then
            AES_KEY_LIST=
            list_add AES_KEY_LIST aes_sck_flash.bin
            list_add AES_KEY_LIST aes_vendor_id.bin
            list_add AES_KEY_LIST aes_module_id.bin
            list_add AES_KEY_LIST aes_common_test.bin
            RSA_KEY_LIST=
